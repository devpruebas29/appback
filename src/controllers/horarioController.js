...
const horarioController = {
    ...
    try {
      ...
      for (let i = 0; i < normalizedBloques.length; i += 1) {
        for (let j = i + 1; j < normalizedBloques.length; j += 1) {
          ...
          if (a.dia_semana === b.dia_semana && overlaps(a.hora_inicio, a.hora_fin, b.hora_inicio, b.hora_fin)) {
            return res.status(409).json({
              success: false,
              message: `Hay traslape de bloques en ${a.dia_semana} dentro de la misma carga enviada.`,
            });
          }
        }
      }
      ...
      for (const b of normalizedBloques) {
        const [conflicts] = await connection.query(
          `SELECT h.id, h.dia_semana, h.hora_inicio, h.hora_fin,
                  a.id_docente, a.id_grado, a.id_seccion, a.id_periodo
           FROM horarios h
           JOIN asignaciones a ON a.id = h.id_asignacion
           WHERE h.activo = 1
             AND a.id_periodo = ?
             ...
             AND (
               a.id_docente = ?
               OR (a.id_grado = ? AND IFNULL(a.id_seccion, 0) = IFNULL(?, 0))
             )`,
          [
            asig.id_periodo,
            b.dia_semana,
        ...
        if (conflicts.length > 0) {
          return res.status(409).json({
            success: false,
            message: `Conflicto horario detectado en ${b.dia_semana} ${b.hora_inicio}-${b.hora_fin}.`,
          });
        }
      }
      ...
      await connection.query("DELETE FROM horarios WHERE id_asignacion = ?", [id_asignacion]);

      const values = normalizedBloques.map((b) => [id_asignacion, b.dia_semana, b.hora_inicio, b.hora_fin, b.aula]);

      await connection.query(
        `INSERT INTO horarios (id_asignacion, dia_semana, hora_inicio, hora_fin, aula)
         VALUES ?`,
        [values]
      );
      ...
      return res.status(201).json({
        ...
        bloques_registrados: values.length,
      });
    } catch (error) {
  ...
  async registrarAsistenciaBloqueMasiva(req, res) {
    const connection = await pool.getConnection();
    try {
      const { id_horario, fecha, asistencia } = req.body;
        ...
        await connection.query(
          `INSERT INTO asistencia (id_alumno, id_asignacion, fecha, asistio, observacion)
           ...
           ON DUPLICATE KEY UPDATE
             ...
             observacion = VALUES(observacion)`,
          [row.id_alumno, horario.id_asignacion, fechaFinal, asistioDiario, "Generado desde asistencia por bloques"]
        );
      }
