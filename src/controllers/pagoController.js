...
// Configurar Mercado Pago fuera del controlador si es posible, o usar el servicio existente
// const client = new MercadoPagoConfig({ accessToken: process.env.MP_ACCESS_TOKEN });
const mpClient = require('../services/mercadoPago'); // Usando el servicio existente

const generarPagoCuota = async (req, res) => {
  let connection;
  try {
    const id_persona = req.user.id_persona;
    const { id_cuota } = req.body; // Recibir ID de la cuota
...
const webhookMercadoPago = async (req, res) => {
  ...
  if (process.env.NODE_ENV === 'production') {
    if (!xSignature || !mpWebhookSecret) {
      console.warn('⚠️ Webhook recibido sin firma o secreto no configurado. Denegando.');
    ...
    try {
      ...
      if (calculatedSignature !== v1) {
        console.error('❌ Firma de webhook INVÁLIDA detecada. Intento de fraude bloqueado.');
        return res.status(403).send('Firma inválida');
      }
    } catch (err) {
      console.error('Error validando firma de webhook:', err.message);
  ...
  try {
    ...
    if (type === 'payment') {
      ...
      if (status === 'approved') {
        ...
        if (pagado) {
          console.log(`✅ Cuota ${idCuota} actualizada a Pagada correctamente.`);
        } else {
          console.error(`❌ Error: No se pudo actualizar la cuota ${idCuota} en la BD. Verifique que el ID exista.`);
        }
      } else {
        console.log(`El pago ${paymentId} no está aprobado (estado: ${status}). No se actualiza la cuota.`);
      }
    } else {
