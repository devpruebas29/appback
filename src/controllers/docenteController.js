...
const docenteController = {
    ...
    try {
      const resultado = await withTransaction(async (connection) => {
        const {
          ...
          especialidad,
          grado_academico,
          id_periodo, // Nuevo campo para el periodo académico
          cursos_asignados // Array de { idCurso, idGrado, idSeccion, idPeriodo }
        } = datos;
        ...
        if (cursos_asignados && cursos_asignados.length > 0) {
          for (const { idCurso, idGrado, idSeccion, idPeriodo } of cursos_asignados) {
            // Limpiar IDs para asegurar que sean NULL si no son válidos (Enteros positivos)
            const cleanId = (val) => {
            ...
            const cleanPeriodo = cleanId(idPeriodo || id_periodo); // Fallback al id_periodo del registro principal

            console.log(`DEBUG: Insertando asignación para docente ${id_docente}:`, {
  ...
  async buscarPorDniLocal(req, res) {
    try {
      ...
      console.error("Error al buscar docente localmente:", error);
  ...
  async registrarAsistenciaMasiva(req, res) {
    const connection = await pool.getConnection();
    try {
      let { asistencia } = req.body; // Puede venir como { asistencia: [] } o como []
      const id_persona = req.user.id_persona;

      // Log para diagnóstico
      console.log("DEBUG: registrarAsistenciaMasiva - Body type:", typeof req.body, "IsArray:", Array.isArray(req.body));
      ...
      if (!asistencia || !Array.isArray(asistencia) || asistencia.length === 0) {
        console.log("DEBUG: Validation failed - asistencia array is empty or missing");
      ...
      const idAsignaciones = [...new Set(asistencia.map(a => a.id_asignacion))];
      const [rows] = await connection.query(
        "SELECT id FROM asignaciones WHERE id IN (?) AND id_docente = ?",
        [idAsignaciones, docente.id]
      );

      console.log(`DEBUG: Assignments valid for docente: ${rows.length} of ${idAsignaciones.length}`);
      ...
      // ----------------------------
      // Eliminado el bloqueo de duplicados para permitir actualizaciones (ON DUPLICATE KEY UPDATE se encarga)
      // ----------------------------
      ...
      const values = asistencia.map(a => {
        ...
        return [
          a.id_alumno || a.id_matricula, // Intentar id_alumno primero, fallback a id_matricula
          a.id_asignacion,
          fechaFinal,
          a.estado === 'Presente' || a.estado === 1 || a.asistio === true ? 1 : 0, // Mapear estado a booleano asistio
          a.observaciones || a.observacion || null
        ];
      });

      console.log(`DEBUG: Inserting ${values.length} records into asistencia`);
  ...
  async getCursosYAlumnosAsignados(req, res) {
    ...
    try {
      const { anio } = req.params;
      const id_persona = req.user.id_persona;
      console.log("DEBUG: getCursosYAlumnosAsignados", { anio, id_persona });

      const docente = await DocenteModel.buscarPorIdPersona(connection, id_persona);

      if (!docente) {
        console.log("DEBUG: Docente not found for id_persona", id_persona);
        return res.status(404).json({ success: false, message: "Docente no encontrado." });
      }
      console.log("DEBUG: Found docente", docente.id);
      ...
      console.log(`DEBUG: Found ${asignaciones.length} assignments for year ${anio}`);
      ...
      console.error("Error al obtener cursos y alumnos:", error);
